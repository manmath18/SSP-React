...
const BookingForm = () => {
  ...
  const handlePayNow = async () => {
    try {
      const client_txn_id = `txn_${Date.now()}`; // Generate a unique transaction ID
      const amount = slot?.price; // Use the slot price as the payment amount
  
      const response = await axios.post("http://localhost:10000/initiate-payment", {
        client_txn_id,
        amount,
        user_id: user?._id,
      });
